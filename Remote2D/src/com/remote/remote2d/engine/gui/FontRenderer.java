...
public class FontRenderer {
	...
	public FontRenderer(Font f, boolean useAntiAliasing) {
		...
		cache = new LinkedHashMap<RenderData, Texture>(16, 0.75f, true) {
			...
			protected boolean removeEldestEntry(
					Map.Entry<RenderData, Texture> eldest) {
				if (size() > 16) {
	...
	public BufferedImage createImageFromString(String s, float size, int color) {
		...
		FontRenderContext frc = new FontRenderContext(null, useAntiAliasing,
				false);
		...
		graphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
				useAntiAliasing ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
		...
		for (String str : returnSplit) {
			...
			graphics.drawString(str, 0,
					h + currentY
							- graphics.getFontMetrics(sizedFont).getDescent());// Remember,
																				// the
																				// y
																				// var
																				// in
	...
	public void drawString(String s, float x, float y, float size, int color) {
		...
		Renderer.drawRect(new Vector2(x, y), new Vector2(tex.image.getWidth(),
				tex.image.getHeight()), tex, 0xffffff, 1);
	...
	public void drawCenteredString(String s, int y, float size, int color) {
		...
		drawString(s, DisplayHandler.getDimensions().x / 2 - stringDim[0] / 2,
				y, size, color);
	}
