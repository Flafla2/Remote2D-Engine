...
public class GuiTextField extends Gui {
	...
	public void tick(int i, int j, int k) {
		...
		if (isSelected) {
			ArrayList<Character> typedChars = (ArrayList<Character>) Remote2D
					.getInstance().getLimitedKeyboardList();
			for (char c : paste.toCharArray())
				typedChars.add(c);
			for (int x = 0; x < typedChars.size(); x++) {
				char key = typedChars.get(x);

				boolean intLimit = Character.isDigit(key)
						|| (key == '-' && !text.contains("-"))
						|| limitToDigits != TextLimiter.LIMIT_TO_INTEGER;
				boolean floatLimit = (key == '.' || key == '-' || Character
						.isDigit(key))
						|| limitToDigits != TextLimiter.LIMIT_TO_FLOAT;
				boolean hexLimit = hexList.contains("" + key)
						|| limitToDigits != TextLimiter.LIMIT_TO_HEX;
				boolean maxLimit = text.length() < maxLength || maxLength == -1;

				if ((intLimit && floatLimit && hexLimit && maxLimit)
						|| key == '\b') {
					hasTyped = true;
	...
	public void render(float interpolation) {
		...
		float yPos = pos.y + dim.y / 2
				- Fonts.get("Arial").getStringDim(text, fontsize)[1] / 2;
		Fonts.get("Arial").drawString(
				s + ((isSelected && blink) ? "|" : "") + " ", pos.x + 10, yPos,
				fontsize, 0xffffff);
		if (text.equals("") && !isSelected)
			Fonts.get("Arial").drawString(defaultText, pos.x + 10, yPos,
					fontsize, 0x777777);
	}
