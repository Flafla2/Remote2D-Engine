...
public class ComponentCamera extends Component {
	...
	public void renderBefore(boolean editor, float interpolation) {
		...
		if (editor) {
			Renderer.drawRect(entity.pos, new Vector2(Fonts.get("Arial")
					.getStringDim("CAMERA", 20)[0] + 10, 20), 1, 1, 0, 1);
			Fonts.get("Arial").drawString("CAMERA", entity.pos.x + 5,
					entity.pos.y, 20, 0x000000);
		}
	...
	public void renderAfter(boolean editor, float interpolation) {
		if (editor) {
			Vector2 dim = DisplayHandler.getGameDimensions();
			Renderer.drawLineRect(new Vector2(entity.pos.x - dim.x / 2,
					entity.pos.y - dim.y / 2), dim, 0, 0, 1, 1);
		}
	}
