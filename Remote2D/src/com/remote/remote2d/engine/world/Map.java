...
public class Map implements R2DFileSaver {
	...
	public void drawGrid(float interpolation) {
		camera.renderBefore(interpolation);
		Vector2 pos = camera.pos.subtract(new Vector2(Gui.screenWidth() / 2,
				Gui.screenHeight() / 2));
		...
		for (int x = 0; x < Gui.screenHeight() / camera.scale / gridSize + 2; x++) {
			...
			Renderer.drawLine(currentPos,
					new Vector2((Gui.screenWidth()) / camera.scale + gridSize
							* 2 + currentPos.x, currentPos.y), color, alpha);
		...
		for (int y = 0; y < Gui.screenWidth() / camera.scale / gridSize + 2; y++) {
			...
			Renderer.drawLine(currentPos,
					new Vector2(currentPos.x, (Gui.screenHeight())
							/ camera.scale + gridSize * 2 + currentPos.y),
					color, alpha);
	...
	/**
	 * Gives you the correction if you have a moving collider somewhere in the
	 * map.
	 * 
	 * @param coll
	 *            The moving collider (before moving)
	 * @param vec
	 *            Said collider's movement vector
	...
	public Vector2 getCorrection(Collider coll, Vector2 vec) {
		...
		for (int x = 0; x < entities.size(); x++) {
			ArrayList<Collider> elementColliders = entities.get(x)
					.getPossibleColliders(coll, vec);
			if (elementColliders != null) {
				allColliders.addAll(elementColliders);
			}
		}

		// First collision pass: Test to see which element will be hit first,
		// and if they are hit at all
		...
		// Second collsion pass: Get the actual collision vectors of each
		// collider and add it to the main return vector
	 ...
	 */
	public static ArrayList<Collision> sortColliders(
			ArrayList<Collision> collection) {
		...
		for (int x = 0; x < collection.size(); x++) {
			...
			int distancePivot;
			if (odd)
				distancePivot = (collection.get(pivot).max + collection
						.get(pivot + 1).max)
						/ 2
						- (collection.get(pivot).min + collection
								.get(pivot + 1).min) / 2;
			else
				distancePivot = collection.get(pivot).max
						- collection.get(pivot).min;
			Log.debug(x + " " + distancePivot + " " + distanceThis);
			if (distanceThis > distancePivot)// if the shortest correction is
												// longer than the pivot(so
												// shortest time to collision),
												// we set it towards the front
	...
	public Entity getTopEntityAtPoint(Vector2 vec) {
		Entity top = null;
		for (int x = 0; x < entities.size(); x++) {
			if (entities.get(x).pos
					.getColliderWithDim(entities.get(x).getDim())
					.isPointInside(vec))
				top = entities.get(x);
		}
		return top;
	...
	public void saveR2DFile(R2DTypeCollection collection) {
		...
		for (int x = 0; x < entities.size(); x++) {
			...
			for (int y = 0; y < components.size(); y++) {
				R2DTypeCollection cComp = new R2DTypeCollection("component_"
						+ y);
				cComp.setString("className",
						Remote2D.getInstance().componentList
								.getComponentID(components.get(y)));
	...
	public void loadR2DFile(R2DTypeCollection collection) {
		...
		for (int x = 0; x < entityCount; x++) {
			...
			if (e == null)
				...
				entities.removeEntityFromList(e); // Will be moved to the top
													// after we load.
													// This is done in order to
													// keep all Entity pointers
													// intact.
			e.loadR2DFile(c);
			...
			for (int y = 0; y < componentCount; y++) {
				...
				Component comp = Remote2D.getInstance().componentList
						.getComponentWithEntity(cComp.getString("className"), e);
