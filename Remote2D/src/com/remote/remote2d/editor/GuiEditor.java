...
public class GuiEditor extends GuiMenu implements WindowHolder, MapHolder {
	...
	public GuiEditor() {
		...
		DisplayHandler.setStretchType(StretchType.NONE);
	}
	...
	public void initGui() {
		...
		if (inspector == null) {
			inspector = new GuiEditorInspector(new Vector2(0, 20), new Vector2(
					300, screenHeight() - previewHeight - 20), this);
		} else
			inspector.dim = new Vector2(300, screenHeight() - previewHeight
					- 20);

		if (preview == null) {
			preview = new GuiEditorPreview(inspector, new Vector2(0,
					screenHeight() - previewHeight), new Vector2(300,
					previewHeight));
		} else {
		...
		if (heirarchy == null) {
			heirarchy = new GuiEditorHeirarchy(new Vector2(screenWidth() - 300,
					20), new Vector2(300, 300), this);
		} else
		...
		if (browser == null)
			browser = new GuiEditorBrowser(this, new Vector2(
					screenWidth() - 300, 320), new Vector2(300,
					screenHeight() - 320));
		else {
	...
	public void render(float interpolation) {
		...
		if (map == null) {
			int[] size = Fonts.get("Pixel_Arial").getStringDim(
					"Remote2D Editor", 40);
			Fonts.get("Pixel_Arial").drawString("Remote2D Editor",
					screenWidth() / 2 - size[0] / 2,
		...
		if (map != null) {
			for (int x = 0; x < map.getEntityList().size(); x++) {
				...
				if (entity.pos.getColliderWithDim(entity.getDim())
						.isPointInside(getMapMousePos())) {
					int fontsize = 10;
					String name = entity.name;
					if (Keyboard.isKeyDown(Keyboard.KEY_U))
						name = entity.getUUID();
					int fontdim[] = Fonts.get("Arial").getStringDim(name,
							fontsize);
					Vector2 dim = new Vector2(fontdim[0] + 20, fontdim[1]);
					Vector2 pos = new Vector2(entity.pos.x + entity.dim.x / 2,
							entity.pos.y + entity.dim.y);
					pos = map.worldToScreenCoords(pos);
					pos.x -= fontdim[0] / 2 + 10;
					Renderer.drawRect(pos, dim, 0x000000, 0.5f);
					Fonts.get("Arial").drawString(name, pos.x + 10, pos.y,
							fontsize, 0xffffff);
				}
			}
		}
	...
	public void tick(int i, int j, int k) {
		...
		if (map != null) {
			if (!isWidgetHovered(i, j) || dragPoint != null) {
				if (Mouse.isButtonDown(2)
						|| (Mouse.isButtonDown(0) && (Keyboard
								.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard
								.isKeyDown(Keyboard.KEY_RSHIFT)))) {
					if (dragPoint == null)
						dragPoint = new Vector2(i / map.camera.scale
								+ map.camera.pos.x, j / map.camera.scale
								+ map.camera.pos.y);
					else {
						map.camera.pos = dragPoint.subtract(new Vector2(i, j)
								.divide(new Vector2(map.camera.scale)));
					}
				} else
					dragPoint = null;
				...
				if (deltaWheel > 0 && map.camera.scale < 16)// zoom in, up
					scale *= 2;
				else if (deltaWheel < 0 && map.camera.scale > 0.25)// zoom out,
																	// down
					scale /= 2;
		...
		if (Remote2D.getInstance().hasMouseBeenPressed()
				&& !(getMouseInWindow(i, j) || menu.isMenuHovered(i, j))) {
			if (map != null && !isWidgetHovered(i, j))
				setSelectedEntity(map.getEntityList().indexOf(
						map.getTopEntityAtPoint(getMapMousePos())));
		}
	...
	private boolean isWidgetHovered(int i, int j) {
		return inspector.pos.getColliderWithDim(inspector.dim).isPointInside(
				new Vector2(i, j))
				|| heirarchy.pos.getColliderWithDim(heirarchy.dim)
						.isPointInside(new Vector2(i, j))
				|| browser.pos.getColliderWithDim(browser.dim).isPointInside(
						new Vector2(i, j))
				|| preview.pos.getColliderWithDim(preview.dim).isPointInside(
						new Vector2(i, j));
	}

	public ColliderBox getWindowBounds() {
		return new ColliderBox(new Vector2(0, 20), new Vector2(screenWidth(),
				screenHeight() - 20));
	}
	...
	public void pushWindow(GuiWindow window) {
		...
		} else if (!(windowStack.peek().pos.getColliderWithDim(windowStack
				.peek().dim).isPointInside(new Vector2(new Vector2(Remote2D
				.getInstance().getMouseCoords()).getElements())))) {
	...
	public boolean getMouseInWindow(int i, int j, GuiWindow window) {
		return window.pos
				.getColliderWithDim(window.dim.add(new Vector2(0, 20)))
				.isPointInside(new Vector2(i, j));
	}
	...
	public void confirmOperation(Operation o) {
		GuiWindowConfirmOperation conf = new GuiWindowConfirmOperation(this,
				new Vector2(40, 40), getWindowBounds(), o);
		conf.setSelected(true);
		conf.pos = new Vector2(screenWidth() / 2 - conf.dim.x / 2,
				screenHeight() / 2 - conf.dim.y / 2);
		pushWindow(conf);
	}
	...
	public void insertEntity(Entity e) {
		e.pos = map.screenToWorldCoords(new Vector2(Gui.screenWidth() / 2, Gui
				.screenHeight() / 2));
