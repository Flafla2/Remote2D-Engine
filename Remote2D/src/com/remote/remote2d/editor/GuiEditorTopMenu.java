...
public class GuiEditorTopMenu extends Gui {
	...
	public GuiEditorTopMenu(GuiEditor editor) {
		...
		GuiEditorTopMenuSection file = new GuiEditorTopMenuSection(currentX, 0,
				height, fileContents, "File", this);
		...
		String[] editContents = { "Undo", "Redo", "Create Animated Sprite",
				"Optimize Spritesheet" };
		GuiEditorTopMenuSection edit = new GuiEditorTopMenuSection(currentX, 0,
				height, editContents, "Edit", this);
		if (edit.getEnabled())
			currentX += edit.width;

		edit.keyCombos[0] = new KeyShortcut(new int[] { Keyboard.KEY_Z });
		edit.keyCombos[1] = new KeyShortcut(new int[] { Keyboard.KEY_Y });
		edit.keyCombos[2] = new KeyShortcut(new int[] { Keyboard.KEY_S })
				.setUseShift(true);
		edit.keyCombos[3] = new KeyShortcut(new int[] { Keyboard.KEY_O })
				.setUseShift(true);

		edit.reloadSubWidth();

		String[] worldContents = { "Insert Entity", "Delete Entity",
				"Toggle Grid", "Increase Grid Size", "Decrease Grid Size",
				"Zoom In", "Zoom Out", "Revert Zoom", "Run Map" };
		GuiEditorTopMenuSection world = new GuiEditorTopMenuSection(currentX,
				0, height, worldContents, "World", this);
		if (world.getEnabled())
			currentX += world.width;

		world.keyCombos[0] = new KeyShortcut(new int[] { Keyboard.KEY_E });
		world.keyCombos[1] = new KeyShortcut(new int[] { Keyboard.KEY_DELETE })
				.setMetaOrControl(false);
		world.keyCombos[2] = new KeyShortcut(new int[] { Keyboard.KEY_G })
				.setMetaOrControl(false);
		world.keyCombos[3] = new KeyShortcut(
				new int[] { Keyboard.KEY_RBRACKET }).setMetaOrControl(false);
		world.keyCombos[4] = new KeyShortcut(
				new int[] { Keyboard.KEY_LBRACKET }).setMetaOrControl(false);
		world.keyCombos[5] = new KeyShortcut(new int[] { Keyboard.KEY_EQUALS })
				.setMetaOrControl(false);
		world.keyCombos[6] = new KeyShortcut(new int[] { Keyboard.KEY_MINUS })
				.setMetaOrControl(false);
		world.keyCombos[7] = new KeyShortcut(new int[] { Keyboard.KEY_0 })
				.setMetaOrControl(false);
		world.keyCombos[8] = new KeyShortcut(new int[] { Keyboard.KEY_R });

		world.reloadSubWidth();

		Iterator<Entry<String, Class<?>>> iterator = Remote2D.getInstance().componentList
				.getIterator();
		...
		GuiEditorTopMenuSection component = new GuiEditorTopMenuSection(
				currentX, 0, height, componentContents, "Component", this);
		if (component.getEnabled())
			currentX += component.width;

		String[] windowContents = { "Toggle Fullscreen", "Console", "Exit" };
		GuiEditorTopMenuSection window = new GuiEditorTopMenuSection(currentX,
				0, height, windowContents, "Window", this);
		if (window.getEnabled())
			currentX += window.width;
		window.keyCombos[0] = new KeyShortcut(new int[] { Keyboard.KEY_F });
		window.keyCombos[1] = new KeyShortcut(new int[] { Keyboard.KEY_C })
				.setUseShift(true);

		window.reloadSubWidth();

		String[] devContents = { "Reinitialize Editor", "View Art Asset",
				"Fancypants Collider Test", "Normal Collider Test",
				"1D Perlin Noise", "2D Perlin Noise", "Toggle Wireframe" };
		GuiEditorTopMenuSection dev = new GuiEditorTopMenuSection(currentX, 0,
				height, devContents, "Developer", this);
		if (dev.getEnabled())
			currentX += dev.width;
	...
	public void render(float interpolation) {
		Renderer.drawRect(new Vector2(0, 0),
				new Vector2(screenWidth(), height), 1, 0.2f, 0.2f, 1);
		...
		Fonts.get("Arial").drawString(fps,
				DisplayHandler.getDimensions().x - width, 0, 20, 0xffffff);
	}
	...
	public void tick(int i, int j, int k) {
		...
		for (int x = 0; x < sections.size(); x++) {
			...
			if (selectedSubSec != -1) {
				...
				// Log.info("Selected box: " + secTitle + ">" +
				// secSubTitle+" ");
			}
		}
		...
		if (secTitle.equalsIgnoreCase("File")) {
			if (secSubTitle.equalsIgnoreCase("New Map")) {
				...
				Log.info("Opening!");
				Map newMap = new Map();
				R2DFileManager mapManager = new R2DFileManager(
						"/res/maps/map.r2d", newMap);
				mapManager.read();
				if (editor.getMap() != null)
			...
			if (secSubTitle.equalsIgnoreCase("Create Animated Sprite")) {
				...
				Remote2D.getInstance().guiList
						.push(new GuiOptimizeSpriteSheet());
			...
			if (secSubTitle.equalsIgnoreCase("Toggle Fullscreen")) {
				DisplayHandler.setDisplayMode(Display.getDesktopDisplayMode()
						.getWidth(), Display.getDesktopDisplayMode()
						.getHeight(), !Display.isFullscreen(), false);
			} else if (secSubTitle.equalsIgnoreCase("Console")) {
				editor.pushWindow(new GuiWindowConsole(editor,
						new Vector2(100), new Vector2(400), editor
								.getWindowBounds()));
			...
			if (secSubTitle.equals("Reinitialize Editor")) {
				...
				GuiWindowViewArtAsset window = new GuiWindowViewArtAsset(
						editor, new Vector2(200, 200), editor.getWindowBounds());
				window.setSelected(true);
				editor.pushWindow(window);
			} else if (secSubTitle.equalsIgnoreCase("Fancypants Collider Test")) {
				GuiWindowCollisionTest window = new GuiWindowCollisionTest(
						editor, new Vector2(300, 300), editor.getWindowBounds());
				window.setSelected(true);
				editor.pushWindow(window);
			} else if (secSubTitle.equalsIgnoreCase("Normal Collider Test")) {
				GuiWindowGeneralColliderTest window = new GuiWindowGeneralColliderTest(
						editor, new Vector2(300, 300), editor.getWindowBounds());
				window.setSelected(true);
				editor.pushWindow(window);
			} else if (secSubTitle.equalsIgnoreCase("1D Perlin Noise")) {
				GuiWindowPerlin1D window = new GuiWindowPerlin1D(editor,
						new Vector2(10, 30), editor.getWindowBounds());
				window.setSelected(true);
				editor.pushWindow(window);
			} else if (secSubTitle.equalsIgnoreCase("2D Perlin Noise")) {
				GuiWindowPerlin2D window = new GuiWindowPerlin2D(editor,
						new Vector2(20, 30), editor.getWindowBounds());
				window.setSelected(true);
				editor.pushWindow(window);
			...
			if (secSubTitle.equalsIgnoreCase("Insert Entity")) {
				...
				Remote2D.getInstance().guiList.push(new GuiInGame(editor
						.getMap().copy()));
				...
				editor.getMap().setScaleAroundScreenPoint(
						new Vector2(Gui.screenWidth() / 2,
								Gui.screenHeight() / 2), 1);
		} else if (secTitle.equalsIgnoreCase("Component")) {
			if (editor.getSelectedEntity() != null) {
				editor.getSelectedEntity().addComponent(
						Remote2D.getInstance().componentList
								.getComponentWithEntity(secSubTitle,
										editor.getSelectedEntity()));
				editor.getInspector().setCurrentEntity(
						editor.getSelectedEntity());
			}
		}
	}
