...
public class GuiOptimizeSpriteSheet extends GuiMenu {
	...
	public GuiOptimizeSpriteSheet() {
		...
		removeField = new GuiTextField(new Vector2(10, 50),
				new Vector2(200, 40), 20);
		removeField.limitToDigits = TextLimiter.LIMIT_TO_INTEGER;
		texturePath = new GuiTextField(new Vector2(10, 120), new Vector2(280,
				40), 20);
		savePath = new GuiTextField(new Vector2(10, 270), new Vector2(280, 40),
				20);
	...
	public void initGui() {
		buttonList.clear();
		buttonList.add(bgButton = new GuiButton(0, new Vector2(10, 10),
				new Vector2(240, 40), "Pick Background Color")
				.setDisabled(true));
		buttonList.add(new GuiButton(1, new Vector2(210, 50), new Vector2(80,
				40), "Remove"));
		buttonList.add(new GuiButton(2, new Vector2(5, 170),
				new Vector2(90, 40), "Left"));
		buttonList.add(new GuiButton(3, new Vector2(105, 170), new Vector2(90,
				40), "Mid"));
		buttonList.add(new GuiButton(4, new Vector2(205, 170), new Vector2(90,
				40), "Right"));
		buttonList.add(new GuiButton(5, new Vector2(5, 220),
				new Vector2(90, 40), "Top"));
		buttonList.add(new GuiButton(6, new Vector2(105, 220), new Vector2(90,
				40), "Mid"));
		buttonList.add(new GuiButton(7, new Vector2(205, 220), new Vector2(90,
				40), "Bot"));
		buttonList.add(button = new GuiButton(8, new Vector2(10, 320),
				new Vector2(280, 40), "Done").setDisabled(true));
		buttonList.add(new GuiButton(9, new Vector2(10, 370), new Vector2(280,
				40), "Cancel"));
	}
	...
	public void render(float interpolation) {
		...
		Renderer.drawRect(new Vector2(250, 10), new Vector2(40, 40), bgColor,
				1.0f);
	...
	public void renderBackground(float interpolation) {
		Vector2 iOffset = Interpolator.linearInterpolate(oldOffset, offset,
				interpolation);
		...
		if (Remote2D.getInstance().artLoader.textureExists(texturePath.text)) {
			...
			tex.bind();
			Vector2 dim = new Vector2(tex.image.getWidth(),
					tex.image.getHeight());

			Renderer.startScissor(new Vector2(300, 0), new Vector2(
					screenWidth() - 300, screenHeight()));
			...
			for (int x = 0; x < frameDefiners.size(); x++) {
				ColliderBox coll = (ColliderBox) frameDefiners.get(x)
						.getCollider();
				coll.drawCollider(0xff0000);
				Fonts.get("Logo").drawString("" + x, coll.pos.x + coll.dim.x,
						coll.pos.y + coll.dim.y, 10, 0xff0000);
			}
			...
			if (Mouse.getX() > 300) {
				...
				GL11.glVertex2f(0, y - (y - offset.y) % scale
						+ (bottom ? scale : 0));
				GL11.glVertex2f(screenWidth(), y - (y - offset.y) % scale
						+ (bottom ? scale : 0));

				GL11.glVertex2f(x - (x - offset.x) % scale
						+ (right ? scale : 0), 0);
				GL11.glVertex2f(x - (x - offset.x) % scale
						+ (right ? scale : 0), screenHeight());
				GL11.glEnd();
	...
	public void tick(int i, int j, int k) {
		...
		if (i > 300 && activeDefiner != null) {
			...
			activeDefiner.hover((i - offset.x) / scale + (right ? 1 : 0),
					(j - offset.y) / scale + (bottom ? 1 : 0));
			if (Remote2D.getInstance().hasMouseBeenPressed()) {
				if (isPickingBG) {
					...
					if (tex != null && x < tex.getImage().getWidth()
							&& y < tex.getImage().getHeight()) {
						bgColor = tex.getImage().getRGB(x, y);
					}

					isPickingBG = !isPickingBG;
					bgButton.text = isPickingBG ? "Cancel"
		...
		if (!Remote2D.getInstance().artLoader.textureExists(texturePath.text)
				|| !savePath.text.startsWith("/")
				|| !savePath.text.endsWith(".png")
				|| savePath.text.endsWith("/.png"))
	...
	public void actionPerformed(GuiButton button) {
		if (button.id == 0) {
			...
			BufferedImage image = new BufferedImage(
					(int) (frameSize.x * frameDefiners.size()),
					(int) frameSize.y, BufferedImage.TYPE_INT_ARGB);
				...
				for (int i = (int) startPos.x; i < frameSize.x; i++) {
					for (int j = (int) startPos.y; j < frameSize.y; j++) {
						int rgb = source.getRGB(
								(int) (i - startPos.x + coll.pos.x), (int) (j
										- startPos.y + coll.pos.y));
						image.setRGB((int) (currentPos.x + i),
								(int) (currentPos.y + j), rgb);
					}
				}
			...
			try {
				...
				throw new Remote2DException(e,
						"Error Saving Optimized Sprite Sheet!");
			}
		} else if (button.id == 9) {
			Remote2D.getInstance().guiList.pop();
		}
