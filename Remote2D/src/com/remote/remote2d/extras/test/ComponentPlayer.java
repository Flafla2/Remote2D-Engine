...
public class ComponentPlayer extends Component {
	...
	private Vector2 velocity = new Vector2(0, 0); // Physics stuff - simple
													// velocity and acceleration
	private Vector2 acceleration = new Vector2(0, 2);
	private Vector2 maxVelocity = new Vector2(10, -1); // Max Velocity is
														// required because we
	...
	public void tick(int i, int j, int k) {
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_SPACE)
				&& (state == PlayerState.WALK || state == PlayerState.IDLE))
			velocity.y -= 20;

		Vector2 correction = entity.getMap().getCorrection(
				entity.pos.getColliderWithDim(entity.getDim()),
				new Vector2(velocity.getElements()));
		...
		if (entity.pos.x + entity.getDim().x > right)
			entity.getMap().camera.pos.x += (entity.pos.x + entity.getDim().x)
					- right;
		...
		if (entity.pos.y + entity.getDim().y > bottom)
			entity.getMap().camera.pos.y += (entity.pos.y + entity.getDim().y)
					- bottom;
		if (entity.pos.y < top)
		...
		if (state != oldState) {
			...
			if (state == PlayerState.LAND && anim != null) {
				timerLength = (int) (anim.getFramelength() * anim.getFrames().x * anim
						.getFrames().y);
		...
		testParticles.pos = entity.getMap().screenToWorldCoords(
				new Vector2(i, j));
		if (particleTest)
	...
	public Animation getAnim() {
		...
		if (anim != null)
			anim.flippedX = spriteFacesRight ? (facing == FacingState.LEFT)
					: (facing == FacingState.RIGHT);
		return anim;
	}
	...
	public void renderAfter(boolean editor, float interpolation) {
		...
		if (state == PlayerState.LAND
				&& System.currentTimeMillis() - lastLand >= timerLength
				&& timerLength != -1) {
			state = PlayerState.IDLE;
			timerLength = -1;
		}
		...
		if (anim != null) {
			...
			posVec.x = entity.getPos(interpolation).x + entity.getDim().x / 2
					- anim.getSpriteDim().x / 2;
			posVec.y = entity.getPos(interpolation).y + entity.getDim().y / 2
					- anim.getSpriteDim().y / 2;
